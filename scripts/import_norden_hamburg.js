...
// Path to NordenHamburg file
const filePath = path.join(__dirname, '../NordenHamburg.xlsx');
const workbook = XLSX.readFile(filePath);
const sheetName = workbook.SheetNames[0];
const sheet = workbook.Sheets[sheetName];
const rows = XLSX.utils.sheet_to_json(sheet);

console.log(`Found ${rows.length} rows in NordenHamburg file.`);
...
async function importData() {
    ...
    for (const row of rows) {
        // Headers: ['Name', 'Reviews', 'Anzahl der Reviews', 'Rating', 'Website', 'Phone', 'Adresse', 'Link', 'Farbe']
        ...
        let city = "Hamburg"; // Default fallback
        let zip = "";
        ...
        if (address) {
            ...
            if (zipMatch) {
                ...
                // Fallback for cases like "22850 Norderstedt" without street at end? No usually it's "Street, Zip City"
                // Or heuristic fallback: take last part after comma
        ...
        batch.push({
            ...
            lat: lat,
            lng: lng,
            category: 'Uncategorized', // No category column in this file
            phone: row.Phone,
            website: row.Website,
            color: row.Farbe,
            rating: row.Rating,
            user_ratings_total: row['Anzahl der Reviews'],
            source_file: 'NordenHamburg.xlsx',
        });
