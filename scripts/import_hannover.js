...
// Adjusted path to Hannover.xlsx in the root directory
const filePath = path.join(__dirname, '../Hannover.xlsx');
const workbook = XLSX.readFile(filePath);
const sheetName = workbook.SheetNames[0];
const sheet = workbook.Sheets[sheetName];
const rows = XLSX.utils.sheet_to_json(sheet);

console.log(`Found ${rows.length} rows. Starting import for Hannover...`);
...
async function importData() {
    ...
    for (const row of rows) {
        ...
        let city = "Hannover"; // Default to Hannover
        let zip = "";
        ...
        batch.push({
            ...
            source_file: 'Hannover.xlsx', // Updated source file
        });
