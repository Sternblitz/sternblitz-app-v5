...
// Path to DüsseldorfNeuss.xlsx
const filePath = path.join(__dirname, '../DüsseldorfNeuss.xlsx');
const workbook = XLSX.readFile(filePath);
const sheetName = workbook.SheetNames[0];
const sheet = workbook.Sheets[sheetName];
const rows = XLSX.utils.sheet_to_json(sheet);

console.log(`Found ${rows.length} rows. Starting import for Düsseldorf/Neuss...`);
...
async function importData() {
    ...
    for (const row of rows) {
        ...
        let city = "Düsseldorf"; // Default Fallback
        let zip = "";
        const address = row.Adresse || "";

        if (address) {
            // Regex for German zip codes (5 digits) + city
        ...
        batch.push({
            ...
            source_file: 'DüsseldorfNeuss.xlsx',
        });
