...
console.log(`Found ${rows.length} rows in Anjas file.`);
...
async function importData() {
    ...
    // We fetch ALL because local filtering is safer/easier for concatenation than remote
    // NOTE: This might be heavy if DB is huge, but for <200k it's okay-ish.
    // Optimization: Pagination or stream if too big, but let's stick to this for now.
    ...
    for (const row of rows) {
        // Headers matches FFM/Norden: ['Name', 'Reviews', 'Anzahl der Reviews', 'Rating', 'Website', 'Phone', 'Adresse', 'Link', 'Farbe']
