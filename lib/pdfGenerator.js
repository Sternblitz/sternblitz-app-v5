...
export async function buildPdf(p, sigBytes, priceInfo = {}) {
    ...
    const boxTop = y;
    const boxHeight = 180; // Increased height
    page.drawRectangle({
        x: 50,
        y: boxTop - boxHeight,
        width: width - 100,
    ...
    if (sigBytes?.length) {
        ...
        page.drawImage(png, { x: 50, y: y - 60, width: 140, height: 70 });
    }

    y -= 20;
    draw(`Datum: ${new Date().toLocaleString("de-DE")}`, { x: 50, y, font, size: 10, color: blackColor });
